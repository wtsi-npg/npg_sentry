...
let validateUser = (groups, user) => {
  ...
  return p_groups.then( function( userGroups ) {
    let default_groups = ['everyone', 'open_access'];

    if (!default_groups && !userGroups) {
      return false;
    }

    // TODO user groups as array of arrays
    return groups.every( individual_group => {
      return individual_group.some( g_element => {
         return default_groups.indexOf(g_element) != -1 || (
           userGroups && userGroups.indexOf(g_element) != -1
         );
      });
    });
  });
