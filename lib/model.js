...
module.exports.createToken = function(user, justification) {
  if (anyNotString([user, justification])) {
    return Promise.reject(new Error('createToken: undefined argument(s)'));
  }
...
module.exports.revokeToken = function(user, token, justification) {
  // TODO check that this user owns this token and can revoke it
  if (anyNotString([user, token, justification])) {
    return Promise.reject(new Error('revokeToken: undefined argument(s)'));
  }
