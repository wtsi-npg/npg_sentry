...
let p_collection = dbConn.p_db.then(getCollection(constants.COLLECTION_TOKENS));
...
let checkToken = function checkToken(groups, token) {
  logger.debug('Checking token ' + token + ' authorises bearer');
  try {
...
let checkUser = function checkUser(groups, user) {
  logger.debug('Checking that user ' + user + ' is member of all groups ' +
